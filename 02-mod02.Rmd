...
```{r fig003014, out.width="55%", echo=FALSE, fig.align='center', fig.cap='Typical lines of constant reaction rate corresponding to set of values of \\temperature and conversion for an exothermic reaction', fig.lp='fig:'}
library(png)
library(grid)
...
\[
A \xrightarrow[\substack{k_2}]{\substack{k_1}} B
\]

Assuming $C_{B0} = 0$, $r_{1} = k_{1}\,C_A$, $r_{2} = k_{2}\,C_B$, and
\begin{align*}
	k_1 &= k_{10} \exp\left(-E1/RT\right)\\
	k_2 &= k_{20} \exp\left(-E2/RT\right)
\end{align*}
with $E2>E1$
```
...
More details about this solution could be found on [Python](https://github.com/aliglara/ebook_che5314_ucv/blob/main/codes/lect_013ThermoMaxExothemica)
```
**Work in Progress**
