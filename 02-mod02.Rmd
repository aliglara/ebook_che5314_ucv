...
```{exercise, it003001, name="Operation of a cooled exothermic CSTR"}
  ...
  ```{r fig-margin, fig.margin=TRUE, out.width = "30%", echo=FALSE}
      library(png)
      library(grid)
...
<p align="center">
![Fractional conversion vs volume for various reaction orders](figures/lect007_01.png){width=55%}
</p>

Figure shows clearly the effect of reaction on conversion in a PFR, for zero-order kinetic will obtain higher conversion than other cases. As we expect, for a zero-order kinetic, fractional conversion shows an independent behavior of concentration _A_. That is why a linear function is observed when the fractional conversion is less than one.
...
```{r fig003016, out.width="55%", echo=FALSE, fig.align='center', fig.cap='', fig.lp='fig:'}
library(png)
library(grid)
...
This system of equations was solved in Python, and the results are presented in Figure X.

The reaction's activation energy is greater than the activation energy of the forward reaction from the data given. 
For this reason, figure X shows that at low temperatures, the conversion is scanty because of detained specific reaction rates. 
...
This problem was solved in [Python](https://github.com/aliglara/ebook_che5314_ucv/blob/main/codes/lect_016CSTROptimization.ipynb). The results are presented in Figure W.


In this problem, the first reactor's output, which contains all four components, is fed to the second reactor. It was assumed each CSTR is 200 m$^3$.

Reactor | Conversion $X_A$ | Temperature (K) 
-|-|-
First  | 0.6789 | 341.41  
Second | 0.7303 | 328.28 

This final conversion of 73% should be compared with the 68% conversion attainable is a single 200 m$^3$ reactor operating at 341 K.
If the number of CSTR is increased, the fractional conversion will be improved. However, at some point, the incremental increase in conversion can not justify the increase in capital investment.
```
**Work in Progress**
